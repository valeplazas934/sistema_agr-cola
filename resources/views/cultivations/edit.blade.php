...
@section('content')
<div class="mb-4">
    <a href="{{ route('cultivations.show', $publication) }}" class="text-green-600 hover:underline flex items-center">
...
<div class="bg-white shadow-md rounded-lg p-6">
    ...
    <form method="POST" action="{{ route('cultivations.update', $publication) }}">
        @csrf
        @method('PUT')
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <select name="idCategory" id="idCategory" 
                ...
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" {{ old('idCategory', $publication->idCategory) == $category->id ? 'selected' : '' }}>
                        {{ $category->name }}
                    </option>
                @endforeach
            </select>
